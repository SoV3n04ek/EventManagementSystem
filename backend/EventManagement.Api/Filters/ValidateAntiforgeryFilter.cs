...
namespace EventManagement.Api.Filters
{
    ...
    public class ValidateAntiforgeryFilter : IAsyncActionFilter
    {
        ...
        private readonly ILogger<ValidateAntiforgeryFilter> _logger;

        public ValidateAntiforgeryFilter(
            ...
            ActionExecutionDelegate next)
        {
            ...
            {
                _logger.LogDebug("XSRF: Skipping validation for [AllowAnonymous] endpoint {Path}",
            ...
            {
                _logger.LogDebug("XSRF: Skipping validation for [IgnoreAntiforgeryToken] endpoint {Path}",
                    context.HttpContext.Request.Path);
                await next();
                return;
            }
            ...
            // ── Validate XSRF token ──
            var hasHeader = context.HttpContext.Request.Headers.ContainsKey("X-XSRF-TOKEN");
            var hasCookie = context.HttpContext.Request.Cookies.ContainsKey(".AspNetCore.Antiforgery");
            ...
            try
            {
                ...
                _logger.LogDebug("XSRF: Validation succeeded for {Method} {Path}",
