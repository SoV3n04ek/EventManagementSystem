    ...
    ILogger<AuthController> logger) : ControllerBase
{
    ...
    static readonly CookieOptions AuthCookieOptions = new()
    {
        HttpOnly = true,
        Secure = true,
    ...
    {
        logger.LogInformation("Registering new user: {Email}", registerDto.Email);
    ...
    {
        logger.LogInformation("Login attempt for user: {Email}", loginDto.Email);
        ...
        int expireHours = 24; // Matches JWT expiry from appsettings.json
        ...
        logger.LogInformation("Login successful for {Email} — XSRF token regenerated", loginDto.Email);
